...
export class RowListPPContainer extends AScrollableContainer {
  ...
  addRowNumber(rowNumber: number, y: number) {
      const rowNumberContainer = new GenericContainer(0, y, Configuration.cellHeight, Configuration.cellHeight, this.canvas, this.innerContainer);
      rowNumberContainer.setBackgroundColor('#1A1A1A');
      rowNumberContainer.setBorder('#858585', 1);

      const rowNumberText = new TextRenderer(rowNumber.toString(), this.canvas, rowNumberContainer);
  ...
  addAllRowNumbers() {
      ...
      CppRef.cpp.allDisplayProjects.forEach(project => {
          ...
          project.users.forEach(user => {
              this.addRowNumber(rowNumber, project.y + user.y + Configuration.cellHeight);
