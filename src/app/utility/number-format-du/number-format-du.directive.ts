...
@Directive({ selector: "[number-format-directive-du]" })
export class NumberFormatDuDirective {
  @Input() allowDecimals: boolean = true;
  @Input() allowSign: boolean = true;
  @Input() decimalSeparator: string = ".";
  @Input() decimalCommaSeparator: string = ",";

  previousValue: string = "";
  private value: any; // store the "real" value
   ...
   * @param hostElement
   */
  constructor(private hostElement: ElementRef, private decimal: DecimalPipe) {
  ...
  @HostListener("keyup", ["$event"]) onKeyUp(e: KeyboardEvent) {
    if (e.target["value"] !== "") {
      if (
        ...
        e.target["value"] = this.decimal.transform(
          e.target["value"].toString().replace(/\s/g, "").replace(",", "."),
          "1.0-2",
          "fr"
        );
      }
  ...
  @HostListener("keydown", ["$event"]) onKeyDown(e: KeyboardEvent) {
    ...
    let separatorCommaExists = originalValue.includes(
      this.decimalCommaSeparator
    );
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      ...
      if(this.value == 0 || this.value == '0.00' || this.value == '0,00') {
        ...
        this.el.value = this.decimal.transform(
          this.value.replace(/\s/g, "").replace(",", "."),
          "1.2-2",
          "fr"
        );
      }
  ...
  onFocus(e) {
    const elementValue = this.el.value;

    const newValue =
      elementValue.replace(/\s/g, "").replace(",", ".") != this.value
        ? elementValue
        : this.value;

    this.el.value = newValue != 0 ? newValue : "";
    this.el.select();
  }
}