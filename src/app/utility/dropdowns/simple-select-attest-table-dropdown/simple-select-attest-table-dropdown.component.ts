...
@Component({
  selector: 'app-simple-select-attest-table-dropdown',
  templateUrl: './simple-select-attest-table-dropdown.component.html',
  styleUrls: ['./simple-select-attest-table-dropdown.component.css']
})
export class SimpleSelectAttestTableDropdownComponent implements OnInit {
  ...
  OndocumentClick(event: MouseEvent){
    ...
    newAtaChanged(item, selected, i3) {
      ...
      if(this.type === 'hours-editor' || this.type === 'minutes-editor'){
        this.emitSelectChanged.emit({
          item: item,
          moment: selected,
          i3: i3,
      ...
      if (this.type === 'moment-editor' && item.parent != '0' ) {
          ...
          selected.ProjectID = item.ProjectID ? item.ProjectID : selected.ProjectID;
          selected.projectName = item.CustomName ? item.CustomName : selected.projectName;
          selected.project_name = item.CustomName ? item.CustomName : selected.project_name;
          this.selected.ProjectID =  item.ProjectID ? item.ProjectID : selected.ProjectID;
          this.selected.projectName = item.CustomName ? item.CustomName : selected.projectName;
          this.selected.project_name = item.CustomName ? item.CustomName : selected.project_name;
