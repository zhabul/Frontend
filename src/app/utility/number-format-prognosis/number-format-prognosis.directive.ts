...
@Directive({ selector: "[number-format-prognosis-directive]" })
export class NumberFormatPrognosisDirective {
  @Input() allowDecimals: boolean = true;
  @Input() allowSign: boolean = true;
  @Input() decimalSeparator: string = ".";
  @Input() decimalCommaSeparator: string = ",";
  @Input('resetValue') set setResetValue(value: number) {
    if (value > 0) {
      this.value = 0;
      this.el.value = '0';
    }
  }
  @Input('setValue') set setValue(value: number) {
    this.value = value;
    this.el.value = `${value}`;
  }
  ...
  onFocus(e) {
    this.el.value = !this.value || this.value == "" ? "0,00" : this.value;
  }
}
